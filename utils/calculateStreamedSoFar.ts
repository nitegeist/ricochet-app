...
const calculateStreamedSoFar = (balance: string, balanceTimestamp: string, flowRate: string) => {
	...
	return flowRateBigNumber.isZero()
		...
		: ethers.utils.formatEther(
				balanceBigNumber.add(currentTimestampBigNumber.sub(balanceTimestampBigNumber).mul(flowRateBigNumber).div(1000))
		  );
};
